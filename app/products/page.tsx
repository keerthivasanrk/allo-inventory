  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div className="space-y-1">
        ...
        {loading ? (
          <div className="h-4 w-64 animate-pulse rounded bg-slate-200" aria-hidden="true" />
        ) : (
          <p className="flex items-center gap-1.5 text-sm text-slate-500">
            {productCount !== undefined && (
              <span className="font-medium text-slate-700">{productCount} products</span>
            )}
            {warehouseLabel && (
              <>
                <span>·</span>
            ...
            <SelectContent>
              {warehouses.map((w) => (
                <SelectItem key={w.id} value={w.id}>
                  ...
                  <span className="ml-1.5 text-slate-400 text-xs">{w.location}</span>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
