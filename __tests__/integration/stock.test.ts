...
// Grab Decimal from the real module before vi.mock() intercepts @prisma/client
const { Decimal } = await vi.importActual<typeof import('@prisma/client')>('@prisma/client')
...
vi.mock('@prisma/client', async () => {
  ...
  return {
    ...actual,
    Decimal: actual.Decimal,  // explicitly pass through so top-level import resolves
...
describe('StockService Integration Tests', () => {
  ...
  describe('getProductsWithStock()', () => {
    ...
    it('throws ValidationError when warehouseId is omitted', async () => {
      await expect(StockService.getProductsWithStock('')).rejects.toThrow(ValidationError)
    })
  })
  ...
  describe('releaseStock()', () => {
    it('returns true on valid lock acquisition and reduction', async () => {
      mockTx.mockImplementation(async (callback: any) => {
        const tx = {
          $queryRaw: vi.fn().mockResolvedValue([{ id: 'stock_1', totalUnits: 10, reservedUnits: 5 }]),
    ...
    it('returns false and skips update if reduction causes an underflow', async () => {
      mockTx.mockImplementation(async (callback: any) => {
        const tx = {
          $queryRaw: vi.fn().mockResolvedValue([{ id: 'stock_1', totalUnits: 10, reservedUnits: 2 }]),
          stock: { update: mockUpdate },
        }
        return callback(tx)
      })
    ...
    it('throws ValidationError for bad input quantities', async () => {
      await expect(StockService.releaseStock('prod_123', 'wh_abc', -1)).rejects.toThrow(ValidationError)
      await expect(StockService.releaseStock('prod_123', 'wh_abc', 2.5)).rejects.toThrow(ValidationError)
    })
  })
})
